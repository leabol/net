...
public:
    explicit InetAddress(std::string host, std::string port)
        ...
        hints_.ai_socktype = SOCK_STREAM;   //Specify the required socket type to return the address and protocol information.
    ...
    explicit InetAddress(std::string port)
        : hostname_("") ,port_(std::move(port)) {
        hints_.ai_flags = AI_PASSIVE | AI_ADDRCONFIG;   // 服务端：绑定所有本地地址；仅返回本机可达协议族
    ...
    // TODO: 可改进 - 暴露迭代接口（begin()/end()）或返回 std::vector<addrinfo> 供调用方枚举候选地址。
...
private:
    void resolve(){
        ...
        // TODO: 可改进 - getaddrinfo 为阻塞调用，必要时考虑超时/取消（例如使用 glibc 的 getaddrinfo_a 或外部超时机制）。
